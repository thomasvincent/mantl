...
def test_vault_secrets_integration(nomad_client, vault_client):
    ...
    assert vault_client.sys.read_health_status(
        method='GET')['initialized'], "Vault is not initialized"
    ...
    # Check if Nomad has access to Vaul
    ...
    try:
        ...
        secret = vault_client.secrets.kv.v2.read_secret_version(
            path="mantl/nomad-test", mount_point="kv")

...
def lvm_profile_dir():
    return check_output(["lvm", "config", "config/profile_dir"]
                        ).splitlines()[0].split("=")[1].replace('"', "")
...
def process_vg(sec, params):
    ...
    for each in pvs():
        if each['pv_name'] in dev_list and each['vg_name'] != name:
            fail(
                "Device {} is already in {} volume group.".format(
                    each['pv_name'],
                    each['vg_name']))
    ...
    if this:
        devs_to_remove = current_devs - dev_lis
        ...
        subprocess.check_call([VGCREATE_CMD] + vgoptions +
                              ['-s', str(pesize), name] + list(dev_list))
...
def process_fs(sec, params):
    ...
    wait_for_device(dev)
    try:
        exist_fs = check_output(["blkid", "-c", "/dev/null", "-o",
                                "value", "-s", "TYPE", dev]).strip()
...
def process_thin(sec, params):
    ...
    if this:
        ...
        convert.extend(["--thinpool", "{}/{}".format(vg, data),
                       "--poolmetadata", "{}/{}".format(vg, meta)])
        check_call(convert)
    ...
    with closing(open(conf, "w")) as f:
        f.write(
            """MANTL_STORAGE_OPTIONS=--storage-driver devicemapper --storage-opt dm.thinpooldev=/dev/mapper/{mapper} {extra}""".format(

...
def configure_callback(conf):
    ...
    log_verbose(
        'true',
        'mesos-master plugin configured with host = %s, port = %s, verbose logging = %s, version = %s, instance = %s' %
        (host,
         port,
         verboseLogging,
         version,
...
def fetch_stats():
    for conf in CONFIGS:
        try:
            ...
            collectd.error(
                'mesos-master plugin: Error connecting to %s - %r' %
                (conf['mesos_url'], e))
            return None
...
def parse_stats(conf, json):
    ...
    if elected_result == 1:
        ...
        log_verbose(
            conf['verboseLogging'],
            'This mesos master node is not elected leader so not writing data.')
        return None
